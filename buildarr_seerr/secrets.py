...
"""Seerr plugin secrets file model."""
...
from .exceptions import SeerrAPIError, SeerrSecretsUnauthorizedError
from .types import SeerrApiKey, SeerrProtocol

if TYPE_CHECKING:
    from typing_extensions import Self

    from .config import SeerrConfig

    class _SeerrSecrets(SecretsPlugin[SeerrConfig]):
        pass

else:

    class _SeerrSecrets(SecretsPlugin):
        pass


class SeerrSecrets(_SeerrSecrets):
    hostname: NonEmptyStr
    port: Port
    protocol: SeerrProtocol
    ...
    @classmethod
    def get(cls, config: SeerrConfig) -> Self:
    ...
    def get_from_url(
        ...
        try:
            ...
            if err.status_code in (HTTPStatus.UNAUTHORIZED, HTTPStatus.FORBIDDEN):
                raise SeerrSecretsUnauthorizedError(
                    (
                        f"Incorrect API key for the Seerr instance at '{host_url}'. "
        ...
        try:
            ...
            raise SeerrSecretsUnauthorizedError(
                f"Unable to find Seerr version in status metadata: {status}",
            ) from None
        except TypeError as err:
            raise SeerrSecretsUnauthorizedError(
                f"Unable to parse Seerr status metadata: {err} (metadata object: {status})",
            ) from None
        return cls(
            hostname=cast(NonEmptyStr, hostname),
            port=cast(Port, port),
            protocol=cast(SeerrProtocol, protocol),
            url_base=url_base,
            api_key=cast(SeerrApiKey, api_key),
            version=version,
        )
