...
class SeerrUsersSettings(SeerrConfigBase):
    """
    These settings change the behaviour for how Seerr allows logins
    ...
    """
    Allow Jellyfin users to sign in without first being imported to Seerr.
    """
    ...
    * `manage-issues` - Grant permission to manage media issues.
    ...
    def from_remote(cls, secrets: SeerrSecrets) -> Self:
        ...
        default_quotas: Dict[str, Dict[str, int]] = remote_attrs.get("defaultQuotas") or {}
        remote_attrs.pop("defaultQuotas", None)
        for category, local_category in (("movie", "movie"), ("tv", "series")):
            remote_attrs[f"{category}QuotaLimit"] = default_quotas.get(category, {}).get(
                "quotaLimit",
                cls.__fields__[f"global_{local_category}_request_limit"].default,
            )
            remote_attrs[f"{category}QuotaDays"] = default_quotas.get(category, {}).get(
                "quotaDays",
