...
class Sonarr(ArrBase):
        ...
        remote_map.extend(
            ...
            (
                ...
                "tags",
                {
                    "encoder": lambda v: sorted(
                        (tag_ids[tag] if isinstance(tag, str) else tag) for tag in v
                    ),
                },
            ),
            (
            ...
            (
                ...
                "activeAnimeProfileId",
                {
                    ...
                    "set_if": bool,
                    "encoder": lambda v: (
                        quality_profile_ids[v]
                        if quality_profile_ids and isinstance(v, str)
                        else v
                    ),
                },
            ),
    ...
    def _get_api_metadata(self, secrets: SeerrSecrets, api_key: str) -> Dict[str, Any]:
        return api_post(
            secrets,
    ...
    def _delete_remote(self, secrets: SeerrSecrets, service_id: int) -> None:
        api_delete(secrets, f"/api/v1/settings/sonarr/{service_id}")


class SonarrSettings(SeerrConfigBase):
    """
    ...
    def update_remote(
        ...
        for service_name, service in self.definitions.items():
            ...
            root_folders: Set[str] = set(
                api_rootfolder.get("path")
                for api_rootfolder in (api_metadata.get("rootFolders") or [])
                if api_rootfolder.get("path")
            )
    ...
    def _resolve_(self, secrets: SeerrSecrets) -> None:
